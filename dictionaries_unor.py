class dictionaries():
	...
	def sub_dag(self,n):
		...
		for e in self.edges:
			if self.src[e] in sub_nodes:
				sub_nodes = sub_nodes.union(self.tar[e])
		sub_edges = set()
		for e in self.edges:
			if self.src[e] in sub_nodes:
	...
	def edge_below(self,v,index):
		_,multi_sub_edges,multi_sub_src,_,_,_ = self.multi_subs()
		...
		for n in self.nodes:
			...
			for e in self.edges:
				if e in sub_edges[n]:
					...
					sub_tent_label[n][e] = {}
					for v in self.tar[e]:
						sub_tent_label[n][e][v] = self.tent_label[e][v]
		return sub_nodes,sub_edges,sub_src,sub_tar,sub_label,sub_tent_label
	...
	def multi_subs(self):
		...
		for n in self.nodes:
			...
			if self.out_degree(n) > 1:
				...
				for i in range(len(graphs)):
					...
					for e in self.edges:
						if e in multi_sub_edges[n][i]:
							...
							multi_sub_tent_label[n][i][e] = {}
							for v in self.tar[e]:
								multi_sub_tent_label[n][i][e][v] = self.tent_label[e][v]
			else:
				graph = self.sub_dag(n)
				_,_,sub_src,sub_tar,sub_label,sub_tent_label = self.subs()
				...
				multi_sub_tent_label[n][0] = sub_tent_label[n]
		return multi_sub_nodes, multi_sub_edges,multi_sub_src, multi_sub_tar,multi_sub_label,multi_sub_tent_label

	def reentrancy(self):
		reentrant = {}
		multi_sub_nodes, multi_sub_edges,_,_,_,_ = self.multi_subs()
		for n in self.nodes:
			reentrant[n] = {}
